		...
		if(is_array($field)) {
			foreach ($field as $key => $value){
				...
				if($strlen > 0) {
					...
					if(!(bool)preg_match("/^([a-zA-Z])+$/i",  $this->request[$value])){
			...
			foreach ($this->request as $key => $value) {
				...
				if($strlen > 0) {
					...
					if(!(bool)preg_match("/^([a-zA-Z])+$/i",  $value)){
			...
			if($strlen > 0) {
				if((bool)preg_match("/^([a-zA-Z])+$/i",  $this->request[$field])) {
					return true;
				} else {
		...
		if(is_array($field)) {
			foreach ($field as $key => $value){
				...
				if($strlen > 0) {
					...
					if(!(bool)preg_match("/^([a-zA-Z0-9])+$/i",  $this->request[$value])){
			...
			if($strlen > 0) {
				...
				if((bool)preg_match("/^([a-zA-Z0-9])+$/i",  $this->request[$field])){
					return true;
				} else {
