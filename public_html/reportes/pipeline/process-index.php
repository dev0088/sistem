<?php
	...
	$ClientesOperando 		= 	"SELECT *  FROM `clientes` where estatus_id = 8";

    $arrOperandoRes 	= $conn->query($ClientesOperando );

    if ( $arrOperandoRes ) 
	{
        while( $row = $arrOperandoRes->fetch_assoc() ) 
		{
            $arrOperando[]  = $row;
			// $arrJson[]			 = array('x' => $row['description'], 'y' => $row['tn']);
        }
    ...
    $OficializadoTable = "SELECT *  FROM `clientes` where estatus_id = 9";
    $arrOficializadoRes =  $conn->query($OficializadoTable );

    if ( $arrOficializadoRes )  
	{
        while( $row = $arrOficializadoRes->fetch_assoc() ) 
		{
            $arrOficializado[]  = $row;
			// $arrJson[]			 = array('x' => $row['description'], 'y' => $row['tn']);
        }
	...
	else 
	{
     ...
     $clientesporOperarTable = "SELECT *  FROM `clientes` where estatus_id = 10";
    $arrclientesporOperarRes =  $conn->query($clientesporOperarTable );

    if ( $arrclientesporOperarRes ) 
	{
        while( $row = $arrclientesporOperarRes->fetch_assoc() ) 
		{
            $arrclientesporOperar[]  = $row;
			// $arrJson[]			 = array('x' => $row['description'], 'y' => $row['tn']);
        }
    } 
	else 
	{
     ...
     $ClientessinTable = "SELECT *  FROM `clientes` where estatus_id = 11";
    $arrClientessinRes =  $conn->query($ClientessinTable );

    if ( $arrClientessinRes ) 
	{
        while( $row = $arrClientessinRes->fetch_assoc() ) 
		{
            $arrClientessin[]  = $row;
			// $arrJson[]			 = array('x' => $row['description'], 'y' => $row['tn']);
        }
