...
/****************************************FORM VALIDATION END*****************************************/	
				...
				{	
					...
					if ($postActionStep > 1) 
					{
						...
						$query = "UPDATE `{$stepData['table']}` SET " . $setQ . " WHERE `dg_id` = {$insertId} ";
					} 
					elseif ($postActionStep == 1) 
					{
						...
						$query = "UPDATE `{$stepData['table']}` SET " . $setQ . " WHERE `id` = {$insertId} ";
					}
					$queryRes 	= $conn->query($query);
					if ($queryRes) 
					{
						$nextStepId = $postActionStep + 1;
						header("Location: modificar.php?".$converter->encode("id=".$insertId."=".$nextStepId));
	...
	if (($insertId && count($continue) == 0) || isset($_SESSION['m'])) 
	{
		if($currStepId == 1)
		{
			$existingEntityQ = "SELECT * FROM `{$stepsData[$currStepId]['table']}` WHERE `id` = {$insertId} LIMIT 1 ";
		}
		else
		{
			$existingEntityQ = "SELECT * FROM `{$stepsData[$currStepId]['table']}` WHERE `dg_id` = {$insertId} LIMIT 1 ";
		}
