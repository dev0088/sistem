<?php
	...
	{
		$data['id_cita']	=	$_POST['citas_id'];
		$data['notes']		=	$_POST['notes'];	
		$data['id_user']	=	$_SESSION["logged_in"]["id"];
		$data['post_date']	=	date("Y-m-d H:i:s");
		$db->query_insert('user_appointments_notes', $data);
    ...
    $filters = array(
	    ...
	    "telefono1" => (filter_input(INPUT_GET, 'telefono1') ? filter_input(INPUT_GET, 'telefono1') : null),
        "telefono2" => (filter_input(INPUT_GET, 'telefono2') ? filter_input(INPUT_GET, 'telefono2') : null),
        "correo" 	=> (filter_input(INPUT_GET, 'correo') ? filter_input(INPUT_GET, 'correo') : null),
        "notas" 	=> (filter_input(INPUT_GET, 'notas') ? filter_input(INPUT_GET, 'notas') : null),
    );
    ...
    $companiesDataQ 		= 	"SELECT U.*, C.nombre as company, P.nombre as plaza FROM `user_appointments` U
								LEFT JOIN clientes C ON(C.id = U.client_id) 
								LEFT JOIN plaza P ON(P.id = C.plaza_id)";
								
								
    $companiesTotalNumQ 	= 	"SELECT count(U.id) as total_num FROM `user_appointments` U
								LEFT JOIN clientes C ON(C.id = U.client_id) 
								LEFT JOIN plaza P ON(P.id = C.plaza_id))";
    if ( "" !== $filtersSQL ) 
	{
