<?php
    ...
    $companiesTotalNumQ 	= 	"SELECT count(U.id) as total_num FROM `DATOS_GENERALES` U
								...
								/*$cios_names = $db->fetch_all_array("SELECT G.id, F.nombre as facturadoras, S.nombre as suministradoras FROM datos_cio_general G LEFT JOIN facturadoras F ON (F.id = G.facturadora_id) LEFT JOIN suministradoras S ON (S.id = G.suministradoras_id) WHERE G.id = ".$objEmployeeData['id_crs']);*/

	$companiesDataQ 		= 	"SELECT U.*, C.nombre as company, F.nombre as facturadoras, S.nombre as suministradoras FROM `labor` U
								LEFT JOIN clientes C ON(C.id = U.id_customer)
								LEFT JOIN datos_cio_general D ON(D.id = U.id_crs)
								LEFT JOIN facturadoras F ON(F.id = D.facturadora_id)
								LEFT JOIN suministradoras S ON(S.id = D.suministradoras_id)
								 ";
