...
// echo "hello"; die;
	...
	$pageURL 		= APP_URL . 'generador/formulario/index.php';
	$breadcrumbs["Generador"] = '';
	$page_nav["Expedientes"]['sub']["Expedientes"]["active"] 	= true;
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('.btn-add-employee').on('click', function() {
            location.href = "<?= APP_URL ?>generador/formulario/home.php?page=insert<?= isset($company_id) ? "&company_id=".$company_id : "" ?>";
        });
		
        $('.btn-edit-employee').on('click', function() {
            var selectedCompanyId = $('.employee-row.selected:first').data('employeeId');
           
            if ( selectedCompanyId ) {
                location.href = "<?= APP_URL ?>generador/formulario/home.php?employee_id=" + selectedCompanyId + "&page=edit<?= isset($company_id) ? "&company_id=".$company_id : "" ?>";
            }
        });
        ...
        $('.btn-view-employee').on('click', function() {
            ...
            if ( selectedCompanyId ) {
                location.href = "<?= APP_URL ?>implementacion/empleados/home.php?employee_id=" + selectedCompanyId + "&page=view";
            }
        });
		
		$('.btn-contract-employee').on('click', function() {
            var 
                selectedCompanyId = $('.employee-row.selected:first').data('employeeId');
            if ( selectedCompanyId ) {
                location.href = "<?= APP_URL ?>generador/formulario/home.php?employee_id=" + selectedCompanyId + "&page=pdf<?= isset($company_id) ? "&company_id=".$company_id : "" ?>";
            }
        });
        ...
        $('.btn-remove-employee').on('click', function() {
            ...
            if ( selectedCompanyId && confirm("Esta seguro de eliminar este generador?") ) 
			{
