<?php
	...
	if(isset($_POST['action']))
	{
        ...
        if (!empty($_POST["people"])){
		    ...
		    $hours 	= $_POST['hours'];
		    $sql="select * from labor where (category='$category') and (pmc_cost='$price') and (id_customer='$customer') and (crsno='$crsno') and (pmc_quantity_of_people='$qtypeople') and (pmc_hours='$hours') and (pmc_cost='$price') and (pmtot_cost='$total');";
		}else{
		    $qtypeople 	= $_POST['quantity'];
		    $hours= 0;
		    $sql="select * from labor where (category='$category') and (pmc_cost='$price') and (id_customer='$customer') and (crsno='$crsno') and (pmc_quantity_of_people='$qtypeople') and (pmc_cost='$price') and (pmtot_cost='$total');";
		}
      ...
      if (mysqli_num_rows($res) > 0) {
        ...
        // if($subcategory==isset($row['subcategory']))
        // {
        //     	$postActionErrorMsg 	= "subcategory already added";
	...
	$query 		= "INSERT INTO `labor`( `id_customer`,`crsno`, `category`, `subcategory`, `pmc_quantity_of_people`, `pmc_hours`, `pmc_cost`, `pmtot_cost`) VALUES ('$customer','$crsno','$category','$subcategory','$qtypeople','$hours','$price','$total') ";
		...
		if ($queryRes) 
		{			
			$postActionNotificationMsg 	= "Subtotal agregado exitosamente";
